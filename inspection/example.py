...
#        if self.page_i == 6:
#            cv2.imshow('dst({0},{1})'.format(self.page_i,self.page_j),temp1_i)
#            cv2.waitKey(0)
...
#        if self.page_i == 6:
#            cv2.imshow('dst({0},{1})'.format(self.page_i,self.page_j),temp1_j)
#            cv2.waitKey(0)
...
#        print self.image_i.shape
        hist_i = cv2.calcHist([self.image_i], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256])
       # print "3D histogram shape: %s, with %d values" % (hist_i.shape, hist_i.flatten().shape[0])
        
        hist_j = cv2.calcHist([self.image_j], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256])
       # print "3D histogram shape: %s, with %d values" % (hist_j.shape, hist_j.flatten().shape[0])
