...
TESTS = os.getenv(
    ...
    str([
        ...
        8161, 8162, 8163, 8164, 8166,
        8167, 8168, 8170, 8171, 8172,
        8173, 8174, 8175, 8176, 8177,
        8178, 8179, 8180, 8181, 111252
    ])
...
class TestViewClassScores(unittest.TestCase):
    ...
    @pytest.mark.skipif(str(8164) not in TESTS, reason='Excluded')
        ...
        while(bar < scroll_total_size):
            try:
                self.teacher.driver.find_element(
                    ...
                    '//span[contains(@class,"average") and ' +
                    'not(contains(text(),"---"))]')
                break
            except (NoSuchElementException,
                    ElementNotVisibleException,
                ...
                bar += scroll_width
                if scroll_total_size <= bar:
                    print("No HWs with period averages")
                    raise Exception
                # drag scroll bar instead of scrolling
                actions = ActionChains(self.teacher.driver)
                actions.move_to_element(scroll_bar)
    ...
    def test_teacher_naviget_a_hw_review_using_question_breadcrumbs_8172(self):
        ...
        while(bar < scroll_total_size):
            try:
                ...
                assert('metric' in self.teacher.current_url()), \
                    'Not viewing reading assignment summary'
                break
            except (NoSuchElementException,
                    ElementNotVisibleException,
    ...
    def test_teacher_see_a_students_work_for_a_reading_assignment_8179(self):
        ...
        while(bar < scroll_total_size):
            try:
                ...
                bar += scroll_width
                if scroll_total_size <= bar:
                    print("No students who have done a reading for this class")
                    raise Exception
                # drag scroll bar instead of scrolling
                actions = ActionChains(self.teacher.driver)
                actions.move_to_element(scroll_bar)
    ...
    def test_teacher_view_a_students_work_for_a_homework_assignment_8180(self):
        ...
        while(bar < scroll_total_size):
            try:
                ...
                bar += scroll_width
                if scroll_total_size <= bar:
                    print("No students who have done a hw for this class")
                    raise Exception
                # drag scroll bar instead of scrolling
                actions = ActionChains(self.teacher.driver)
                actions.move_to_element(scroll_bar)
    ...
    def test_teacher_view_students_work_for_an_external_assignment_8181(self):
        ...
        while(bar < scroll_total_size):
            try:
                ...
                bar += scroll_width
                if scroll_total_size <= bar:
                    print("No students have done an external for this class")
                    raise Exception
                # drag scroll bar instead of scrolling
                actions = ActionChains(self.teacher.driver)
                actions.move_to_element(scroll_bar)
